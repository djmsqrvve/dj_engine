...
use dj_engine::editor::state::{EditorUiState, FeatureGrid, EditorView};
#[test]
fn test_headless_initialization() {
    ...
    assert!(app.world().contains_resource::<EditorUiState>(), "EditorUiState should be initialized");
    
    // Verify FeatureGrid resource exists
    assert!(app.world().contains_resource::<FeatureGrid>(), "FeatureGrid should be initialized");
    
    // Check initial state
    let ui_state = app.world().resource::<EditorUiState>();
    assert_eq!(ui_state.global_view, EditorView::Core, "Default global view should be Core");
}
...
fn test_feature_grid_population() {
    ...
    // Verify ecosystems
    assert!(!grid.ecosystems.is_empty(), "Feature Grid should have ecosystems");

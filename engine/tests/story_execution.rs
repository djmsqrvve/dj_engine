...
use dj_engine::story_graph::types::{StoryNode, StoryGraph, GraphExecutor, StoryFlags, StoryGraphLibrary, ExecutionStatus};
...
fn test_subgraph_execution() {
    ...
    assert!(flags.get("in_subgraph"), "Should be in subgraph and have processed s0");
    ...
    let flags = app.world().resource::<StoryFlags>();
    assert!(flags.get("back_in_main"), "Should have returned to main graph and processed m1");
...
fn test_stack_depth() {
    ...
    let c_wait = graph_c.add(StoryNode::Wait { duration: 1.0, next: None });
    ...
    let b0 = graph_b.add(StoryNode::SubGraph { graph_id: "C".into(), next: None });
    graph_b.set_start(b0);
    
    let mut graph_a = StoryGraph::new();
    let a0 = graph_a.add(StoryNode::SubGraph { graph_id: "B".into(), next: None });
    ...
    let mut executor = app.world_mut().resource_mut::<GraphExecutor>();
    executor.wait_timer.tick(std::time::Duration::from_secs_f32(1.1));
    
    // Frame 2: C finishes -> Pops B -> B finishes -> Pops A -> A finishes
    app.update();
    let executor = app.world().resource::<GraphExecutor>();
    assert_eq!(executor.stack.len(), 0, "Stack should be empty after completion");

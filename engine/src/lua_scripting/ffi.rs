...
fn lock_state<'a, T>(result: std::result::Result<std::sync::RwLockReadGuard<'a, T>, std::sync::PoisonError<std::sync::RwLockReadGuard<'a, T>>>) -> LuaResult<std::sync::RwLockReadGuard<'a, T>> {
    result.map_err(|_| LuaError::external(crate::prelude::DJEngineError::RuntimeError("State lock poisoned - thread panic detected".into())))
}
...
pub fn register_generic_state_api(lua: &Lua, state: SharedGenericState) -> LuaResult<()> {
    ...
    // set_float(key, value)
    let state_set_float = state.clone();
    let set_float = lua.create_function(move |_, (key, value): (String, f32)| {
        if key.len() > MAX_LUA_STRING_LEN {
            return Err(LuaError::external(crate::prelude::DJEngineError::RuntimeError(format!("Key too long (max {} bytes)", MAX_LUA_STRING_LEN))));
        }
        let mut data = lock_state_mut(state_set_float.write())?;
    ...
    let set_string = lua.create_function(move |_, (key, value): (String, String)| {
        if key.len() > MAX_LUA_STRING_LEN || value.len() > MAX_LUA_STRING_LEN {
            return Err(LuaError::external(crate::prelude::DJEngineError::RuntimeError(format!("String too long (max {} bytes)", MAX_LUA_STRING_LEN))));
        }
        let mut data = lock_state_mut(state_set_string.write())?;
    ...
    // get_bool(key) -> bool
    let state_get_bool = state.clone();
    let get_bool = lua.create_function(move |_, key: String| {
        let data = lock_state(state_get_bool.read())?;

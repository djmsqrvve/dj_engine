...
pub fn draw_feature_grid(ui: &mut egui::Ui, world: &mut World) {
    ...
    for x in (-10..10).map(|i| i as f32 * 60.0) {
        for y in (-10..10).map(|i| i as f32 * 52.0) {
            let offset = if (y / 52.0) as i32 % 2 == 0 { 0.0 } else { 30.0 };
            let pos = center + egui::vec2(x + offset, y);
            if rect.contains(pos) {
                draw_hexagon(ui.painter(), pos, 25.0, Color32::TRANSPARENT, Stroke::new(0.5, grid_color));
            }
        }
    }
    
    // Draw CORE hub at center
    draw_hexagon(ui.painter(), center, 50.0, Color32::from_rgb(20, 20, 30), Stroke::new(3.0, COLOR_PRIMARY));
    ui.painter().text(center, egui::Align2::CENTER_CENTER, "CORE", egui::FontId::proportional(18.0), COLOR_PRIMARY);
    ...
    for ecosystem in &feature_grid.ecosystems {
        ...
        draw_hexagon(ui.painter(), eco_center, 40.0, ecosystem.color.linear_multiply(0.2), Stroke::new(2.0, ecosystem.color));
        ui.painter().text(eco_center, egui::Align2::CENTER_CENTER, &ecosystem.icon, egui::FontId::proportional(20.0), ecosystem.color);
        ui.painter().text(
        ...
        for (i, node) in ecosystem.nodes.iter().enumerate() {
            let angle = (i as f32 / ecosystem.nodes.len() as f32) * std::f32::consts::TAU - std::f32::consts::FRAC_PI_2;
            ...
            let stroke_color = if is_enabled { ecosystem.color } else { Color32::DARK_GRAY };
            
            // Connection to ecosystem center
            ui.painter().line_segment([eco_center, node_pos], Stroke::new(1.0, stroke_color.linear_multiply(0.5)));
            
            // Node hexagon
            draw_hexagon(ui.painter(), node_pos, 28.0, fill, Stroke::new(1.5, stroke_color));
            
            // Icon and name
            ui.painter().text(node_pos - egui::vec2(0.0, 5.0), egui::Align2::CENTER_CENTER, &node.icon, egui::FontId::proportional(16.0), Color32::WHITE);
            ui.painter().text(
                node_pos + egui::vec2(0.0, 35.0),
                egui::Align2::CENTER_CENTER,
                &node.name,
                egui::FontId::proportional(10.0),
                if is_enabled { Color32::WHITE } else { Color32::DARK_GRAY },
            );
            
            // Status indicator
            let status_pos = node_pos + egui::vec2(20.0, -20.0);
            let status_color = if is_enabled { Color32::GREEN } else { Color32::RED };
            ...
            if response.hovered() {
                egui::show_tooltip_at_pointer(ui.ctx(), ui.layer_id(), egui::Id::new(&node.id), |ui| {
                    ui.label(RichText::new(&node.name).strong());
                    ui.label(&node.description);
                    ui.label(if is_enabled { "✅ Enabled" } else { "❌ Disabled" });
                });
            }
        }
    }

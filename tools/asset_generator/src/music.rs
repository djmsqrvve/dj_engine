...
pub fn generate_overworld_theme() -> Vec<u8> {
    let mut smf = Smf::new(Header::new(Format::Parallel, midly::Timing::Metrical(480.into())));
    ...
    track0.push(TrackEvent {
        delta: 0.into(),
        kind: TrackEventKind::Midi { channel: 0.into(), message: MidiMessage::ProgramChange { program: 80.into() } },
    });
    
    // Bass line: D2 (38), A2 (45), Bb2 (46), A2 (45) - Ostinato
    let bass_notes = [38, 45, 46, 45];
    for _ in 0..8 { // 8 measures
        for note in bass_notes.iter() {
            // Note On
            track0.push(TrackEvent {
                delta: 0.into(),
                kind: TrackEventKind::Midi { channel: 0.into(), message: MidiMessage::NoteOn { key: (*note).into(), vel: 100.into() } },
            });
            // Note Off (Quarter note = 480 ticks)
            track0.push(TrackEvent {
                delta: 480.into(),
                kind: TrackEventKind::Midi { channel: 0.into(), message: MidiMessage::NoteOff { key: (*note).into(), vel: 0.into() } },
            });
        }
    }
    track0.push(TrackEvent { delta: 0.into(), kind: TrackEventKind::Meta(MetaMessage::EndOfTrack) });
    ...
    track1.push(TrackEvent {
        delta: 0.into(),
        kind: TrackEventKind::Midi { channel: 1.into(), message: MidiMessage::ProgramChange { program: 88.into() } },
    });
    ...
    for _ in 0..2 { // Repeat progression chords twice
        for chord in chords.iter() {
            ...
            for (i, note) in chord.iter().enumerate() {
                track1.push(TrackEvent {
                    ...
                    kind: TrackEventKind::Midi { channel: 1.into(), message: MidiMessage::NoteOn { key: (*note).into(), vel: 80.into() } },
                });
            }
            ...
            for (i, note) in chord.iter().enumerate() {
                 track1.push(TrackEvent {
                    ...
                    kind: TrackEventKind::Midi { channel: 1.into(), message: MidiMessage::NoteOff { key: (*note).into(), vel: 0.into() } },
                });               
            }
        }
    }
    track1.push(TrackEvent { delta: 0.into(), kind: TrackEventKind::Meta(MetaMessage::EndOfTrack) });
    ...
    let mut track2 = Vec::new();
     track2.push(TrackEvent {
        delta: 0.into(),
        kind: TrackEventKind::Midi { channel: 2.into(), message: MidiMessage::ProgramChange { program: 13.into() } },
    });
    ...
    for _ in 0..8 {
        for note in melody_notes.iter() {
            track2.push(TrackEvent {
                ...
                kind: TrackEventKind::Midi { channel: 2.into(), message: MidiMessage::NoteOn { key: (*note).into(), vel: 90.into() } },
            });
            track2.push(TrackEvent {
                delta: 240.into(),
                kind: TrackEventKind::Midi { channel: 2.into(), message: MidiMessage::NoteOff { key: (*note).into(), vel: 0.into() } },
            });
        }
    }
    track2.push(TrackEvent { delta: 0.into(), kind: TrackEventKind::Meta(MetaMessage::EndOfTrack) });

...
fn run_startup_scripts(lua_ctx: Res<LuaContext>) {
    ...
    let possible_paths = [
        "assets/scripts/bridge_entry.lua",
        "games/dev/bridge/assets/scripts/bridge_entry.lua",
    ];

    for path in possible_paths {
        if let Ok(content) = std::fs::read_to_string(path) {
            info!("Loading HELIX BRIDGE script: {}", path);
            if let Err(e) = lua.load(&content).exec() {
                error!("Error running Helix Bridge: {}", e);
            }
            return;
        }
    }

    warn!("No bridge scripts found");
}
